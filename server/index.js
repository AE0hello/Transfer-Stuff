...
// custom boss spawner
const spawnFallenBosses = () => {
    if (global.arenaClosed) return (
      util.log("Failed to spawn fallen boss, (Reason: Arena Closed).")
    );
    if (!c.ASSAULT && !c.ASSAULT2) {
      let choose = [Class.fallenBooster, Class.fallenOverlord, Class.fallenAnnihilator];
      let o = new Entity(ran.choose(room.nestSpawn).loc);
      o.define(choose[Math.floor(Math.random() * choose.length)]);
      o.define({
        VALUE: 50000,
        BODY: {
          FOV: 1,
      ...
      o.team = TEAM_ENEMIES;
      o.isFallen = true;
      sockets.broadcast(o.name + " has spawned!!");
      o.on('dead', () => {
        sockets.broadcast(o.name + " Has been killed!")
      });
    }
  }
// Bring it to life
